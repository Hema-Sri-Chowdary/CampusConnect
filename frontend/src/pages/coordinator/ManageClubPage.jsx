...
export default function ManageClubPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['my-clubs'], queryFn: () => clubsAPI.getMyClub().then(r => r.data) });
  ...
  const club = clubs.find(c => c._id === selectedClubId) || clubs[0];

  useEffect(() => {
    if (club) {
      reset({ description: club.description, contactEmail: club.contactEmail });
    }
  }, [club, reset]);

  const mutation = useMutation({
    mutationFn: ({ id, d }) => clubsAPI.update(id, d),
    onSuccess: () => { qc.invalidateQueries(['my-clubs']); toast.success('Club updated!'); },
