        ...
        <div className="card p-8">
          ...
          <div className="flex bg-dark-800/60 p-1 rounded-xl border border-dark-700 mb-5">
            {[['student', '🎓 Student'], ['coordinator', '💼 Coordinator']].map(([val, label]) => (
              <label key={val} className={`flex-1 flex items-center justify-center py-2 rounded-lg text-sm font-medium cursor-pointer transition-all ${role === val ? 'bg-primary-500 text-white shadow-glow' : 'text-dark-100 hover:text-white'
                }`}>
                    ...
                    {clubs.map(club => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-left text-sm transition-all font-medium ${isSelected ? 'shadow-md' : 'hover:border-primary-400 hover:bg-primary-500/10'
                            }`}
                        >
                          <span style={{
