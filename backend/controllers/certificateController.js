...
exports.generateCertificate = async (req, res, next) => {
  try {
    ...
    const certificate = await Certificate.create({
      ...
      certificateNumber: certNumber, certificateUrl: `/api/certificates/placeholder/download`,
...
exports.downloadCertificate = async (req, res, next) => {
  try {
    ...
    if (cert.userId._id.toString() !== req.user._id.toString() && req.user.role !== 'admin') {
      return res.status(403).json({ success: false, message: 'Not authorized.' });
    }
    ...
    // Name
    doc.fillColor('#ffffff').fontSize(32).font('Helvetica-Bold').text(cert.userId.name, 0, 225, { align: 'center' });
    ...
    doc.fillColor('#6C63FF').fontSize(22).font('Helvetica-Bold').text(cert.eventId.title, 0, 305, { align: 'center' });
    doc.fillColor('#94a3b8').fontSize(13).font('Helvetica').text(`on ${new Date(cert.eventId.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}`, 0, 340, { align: 'center' });
    
    // Certificate number
    doc.fillColor('#64748b').fontSize(10).text(`Certificate No: ${cert.certificateNumber}`, 0, 400, { align: 'center' });

    // Generate and render QR verification code
    const qrData = `${process.env.CLIENT_URL || 'http://localhost:5173'}/verify-certificate/${cert.qrVerificationCode}`;
    const qrImage = await QRCode.toDataURL(qrData);
    const qrBuffer = Buffer.from(qrImage.replace(/^data:image\/png;base64,/, ""), 'base64');
    
    // Draw QR code bottom right
    doc.image(qrBuffer, doc.page.width - 150, doc.page.height - 150, { width: 80, height: 80 });
    doc.fillColor('#94a3b8').fontSize(8).text('Scan to Verify', doc.page.width - 150, doc.page.height - 65, { width: 80, align: 'center' });
